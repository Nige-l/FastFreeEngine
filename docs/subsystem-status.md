...
> Honest assessment of each subsystem as of Session 114 (2026-03-08).
> 1579 total tests. Ratings reflect actual test coverage, demo usage, and known gaps.
...
- **Deps:** ECS, Renderer
- **Notes:** 128 inline particle pool per emitter. Gravity, color/size interpolation. Used in Breakout demo. No 3D particle system exists (showcase uses geometry workaround).
...
- **Deps:** Camera
- **Notes:** Griess-Hartmann frustum extraction. Used by terrain LOD system.
...
### Screenshot
- **Rating:** Tested
- **Tests:** ~11 (test_screenshot.cpp)
- **Deps:** RHI
- **Notes:** Pixel readback to PNG. Used by screenshot pipeline tooling.
...
- **Notes:** ChunkState machine, background worker thread, GL upload gate, dirty-distance gating. Tested but inherently difficult to test threading edge cases exhaustively.
...
- **Deps:** RHI, Camera
- **Notes:** Reflection FBO (half-res), Fresnel blend, animated UV scroll. Highest test count of any renderer subsystem. WaterManager API.
- **Known gap:** Water surface (2x2m fountain basin) not visible in Level 1 screenshot -- cosmetic, low priority.
...
- **Deps:** GLFW, glad
- **Notes:** Buffer/texture/shader/uniform/draw abstraction. Supports FBOs, multisample FBOs, instanced draw. Workhorse of the engine. API has grown significantly (Phase 7 additions: viewport, texture queries, sub-image update, pixel readback, multisample/blit).

### RHI (Vulkan)
- **Rating:** Experimental
- **Tests:** ~59 (test_rhi_vulkan.cpp)
- **Deps:** volk, VMA, glslc (build-time SPIR-V)
- **Notes:** Compile-time `FFE_BACKEND` selection. Instance/device/swapchain, VMA memory, SPIR-V graphics pipeline, resource manager (256 buf/tex, 32 shaders), depth buffer, Blinn-Phong shader. Has good test count but only covers basic rendering (textured mesh with lighting). Does NOT support: shadows, post-processing, instancing, PBR, skeletal animation, terrain, or any feature added after Phase 8. Significantly behind the OpenGL backend in feature coverage.

### Shader Library
- **Rating:** Tested
- **Tests:** Tested indirectly through subsystem tests (no dedicated test file)
- **Deps:** RHI
- **Notes:** 16 shaders, all GLSL 330 core, inline in shader_library.cpp. Plus 6 SPIR-V shaders for Vulkan. Every new rendering feature adds shaders here.
...
- **Deps:** ECS
- **Notes:** Spatial hash broadphase, AABB/Circle narrow phase, layer/mask filtering, collision callbacks. Well-tested and stable. Used in Pong, Breakout, Collect Stars, Net Arena.

### 3D Physics (Jolt)
- **Rating:** Tested
- **Tests:** ~18 (test_physics3d.cpp) + ~7 (test_physics3d_collisions.cpp) + ~7 (test_physics3d_raycast.cpp)
- **Deps:** Jolt Physics (vcpkg)
- **Notes:** Rigid bodies, collision detection, raycasting. Used in Showcase game. Functional but less battle-tested than 2D physics.
...
- **Notes:** ~225 `ffe.*` bindings. Sandboxed: instruction budget, blocked globals (os, io, loadfile, dofile), path traversal prevention. Highest test concentration in the project. Every demo is a Lua script. The sandbox security tests are extensive.
...
- **Deps:** ENet (vcpkg)
- **Notes:** UDP transport, reliable/unreliable channels. Functional but real-world testing limited to localhost demos.

### Replication
- **Rating:** Tested
- **Tests:** ~24 (test_replication.cpp)
- **Deps:** ECS, Transport
- **Notes:** Snapshot-based entity replication. Good unit test coverage.

### Prediction / Lag Compensation
- **Rating:** Tested
- **Tests:** ~15 (test_prediction.cpp) + ~15 (test_lag_compensation.cpp)
- **Deps:** ECS, Replication
- **Notes:** Client-side prediction with server reconciliation. Hit detection with lag compensation window. Tested in Net Arena demo.

### Lobby System
- **Rating:** Tested
- **Tests:** ~20 (test_lobby.cpp)
- **Deps:** Transport
- **Notes:** Create/join/leave lobby, ready state, game start. Unit tested.
...
### Editor Application
- **Rating:** Tested
- **Tests:** ~96 across 9 test files (command_history, component_add_remove, component_commands, entity_commands, exporter, hierarchy_panel, llm_panel, play_mode, shortcuts)
- **Deps:** ImGui (vcpkg), Core, Renderer
- **Notes:** Standalone application with dockspace layout: hierarchy panel, inspector, viewport with gizmos, build pipeline/exporter. Phase 10 additions: prefab system, visual scripting (11 node types, topological sort), LLM integration panel. Functional MVP but not yet used for real game production.

### Prefab System
- **Rating:** Tested
- **Tests:** ~21 (test_prefab_system.cpp)
- **Deps:** ECS, JSON, Scene Serialisation
- **Notes:** JSON-based prefab loading, ECS instantiation, override support. Security hardened (path traversal, size limits, depth limits).

### Visual Scripting
- **Rating:** Tested
- **Tests:** ~42 (test_visual_scripting.cpp)
- **Deps:** ECS, Editor
- **Notes:** 11 built-in node types, topological sort execution, function-pointer dispatch. 14-step security pipeline. ImGui graph editor with bezier connections. New in Phase 10 M2.

### LLM Integration Panel
- **Rating:** Experimental
- **Tests:** ~18 (test_llm_panel.cpp)
- **Deps:** cpp-httplib (vendored)
- **Notes:** Async HTTP to LLM API, context-aware queries using .context.md files, Lua snippet insertion. New in Phase 10 M3. Requires external LLM API key. Security reviewed (HTTPS enforcement, response cap, API key scrubbing, path traversal prevention). `#ifdef FFE_EDITOR` guarded.
...
### CMake Build System
- **Rating:** Production-ready
- **Tests:** N/A (validated by CI)
- **Deps:** CMake, Ninja, mold, ccache
- **Notes:** Clang-18 + GCC-13 on Linux. MinGW cross-compile for Windows. macOS CI disabled (upstream LuaJIT issue). SPIR-V compilation via glslc for Vulkan shaders.

### GitHub Actions CI
- **Rating:** Tested
- **Tests:** N/A
- **Deps:** GitHub Actions
- **Notes:** Linux Clang-18, Linux GCC-13 matrix. Zero-warning enforcement. All 1579 tests run on every push.
...
| Water | Tested | 56 | Cosmetic visibility issue |
